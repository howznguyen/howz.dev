...
const NavItem = ({isMobile, key, data}: NavItemProps) => {
    ...
    return (isMobile ?
            <>
                ...
                <Link href={data.url ?? '#'} locale={locale} className="rounded-md hover:bg-gray-50">
                ...
                {   
                    ...
                    <button onClick={() => setShowChildMenu(!isShowChildMenu)}>
                        <Icon icon="HiChevronDown" className={`ml-2 transition-all duration-300 ${isShowChildMenu ? 'rotate-180' : ''}`} />
                    </button>
                    
                }
                </div>
                <div className={`dark:text-white list-disc my-1 transition-all duration-500 ${isShowChildMenu ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                    ...
                    && children.map((item: any, index: number) => 
                        <div className="ml-4 p-3 flex items-center " key={index}>
                            ...
                            <Link href={item.url ?? '#'} className="font-semibold" locale={locale}>
                                {item.title}
                            </Link>
                        </div>
                    )}
        ...
        :
            <div className="nav-item mr-3 relative transition-colors dark:text-white hover:dark:text-primary hover:text-primary">
                ...
                >
                    {data.title}
                    {children && children.length > 0 && <Icon icon="HiChevronDown" className="dropdown-icon inline-block ml-2 transition-all duration-300"/>}
                </Link>
