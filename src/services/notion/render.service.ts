...
export class NotionRenderService {
  ...
  static blocksToRenderData(contents: Array<any>): NotionRenderBlock[] {
    return contents.map((content, i) =>
      this.blockToRenderData(content, i, contents)
    ...
    switch (type) {
      ...
      case 'bulleted_list_item':
        return {
          ...
          children: content.has_children ? this.blocksToRenderData(content.children) : [],
          key: index
        };
      ...
      case 'table':
        ...
        const data = content.children?.map((row: any) => {
  ...
  private static richTextToString(richText: Array<any>): string {
    ...
    return richText.map((content: any) => {
   ...
   */
  static calculateReadingTime(contents: Array<any>, wordsPerMinute: number = 200): number {
   ...
   */
  static getContentSummary(contents: Array<any>, maxLength: number = 160): string {
    const textContent = this.extractTextContent(contents);
    ...
    return cutPoint > 0 ? textContent.substring(0, cutPoint) + '...' : truncated + '...';
  }
}
