...
import { cleanText, defaultBlurData, idToUuid } from "@/lib/helpers";
...
export async function queryDatabaseImpl(opts: {
  ...
  try {
    ...
    const retryAfter =
      error?.response?.headers["retry-after"] || error["retry-after"];
...
export async function getBlocks(
  ...
  let data = await retrieveBlockChildren(
    blockId,
    undefined,
    undefined,
    notionToken,
  ...
  for (const block of blocks) {
    ...
    if (block.type === "image") {
      const url =
        get(block, "image.file.url") || get(block, "image.external.url");
      if (url) {
    ...
    if (block.type === "bookmark") {
      ...
      if (url) {
        const bookmark: any = {
          url,
          title: cleanText(get(block, "bookmark.title")) || "Bookmark",
          description: cleanText(get(block, "bookmark.description")) ?? null,
          favicon: undefined,
          imageSrc: get(block, "bookmark.cover") ?? null,
        };
